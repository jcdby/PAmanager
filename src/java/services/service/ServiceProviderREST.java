...
public class ServiceProviderREST {
    ...
    @EJB
    ServiceProviderForPAFacade facade;
    ...
    @Path("{id}")
    @Consumes({"application/xml", "application/json"})
    public void edit(@PathParam("id") Long id, ServiceProviderForPA entity) {
        facade.edit(entity);
    }
    ...
    public ServiceProviderForPA find(@PathParam("id") Long id) {
    ...
    @Path("{from}/{to}")
    @Produces({"application/xml", "application/json"})
    public List<ServiceProviderForPA> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
